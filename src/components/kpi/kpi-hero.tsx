  ...
  const beats = [
    {
      ...
      claim: `${headline.part1} & ${headline.part2}.`,
      body: headline.sentence,
      status: headline.status,
    },
    ...headline.signals.map((sig) => ({
      eyebrow: sig.label,
      claim: `${sig.label} at ${sig.value}${sig.delta ? ` — ${sig.delta}` : ""}.`,
      body: sig.trend === "up" && sig.inverse
        ? `${sig.label} is trending upward, which represents deteriorating performance. Board attention recommended if trend continues.`
        : sig.trend === "down" && !sig.inverse
        ? `${sig.label} is declining quarter-on-quarter. Review underlying drivers before next board meeting.`
        : `${sig.label} is tracking within expected range. Continue monitoring against 8-quarter baseline.`,
      status: (sig.inverse
        ? sig.trend === "up" ? "watch" : "healthy"
        : sig.trend === "down" ? "watch" : "healthy") as "healthy" | "watch" | "action",
    })),
    {
      eyebrow: "Board recommendation",
      claim: headline.status === "action"
        ? "Immediate pricing review warranted."
        : headline.status === "watch"
        ? "Monitor claims trajectory through next quarter."
        : "Performance within acceptable parameters.",
      body: headline.status === "action"
        ? "Loss ratio has breached the 88% action threshold. A board-level pricing review should be initiated before the next quarter closes to prevent margin erosion below the 4% floor."
        : headline.status === "watch"
        ? "Margin compression is not yet at crisis level, but the trend is consistent. A pre-emptive review of premium rate assumptions would be prudent ahead of the FY2026 renewal cycle."
        : "The portfolio is performing within historical norms. Investment in growth initiatives and member retention programs remains appropriate at this margin level.",
      status: headline.status,
    },
  ]
          ...
          <div className="min-w-0">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              {headline.signals.map((sig, i) => {
                ...
                const trendColour = isGood ? "var(--color-positive)" : isBad ? "var(--color-negative)" : "var(--color-text-tertiary)"
                const TrendIcon = sig.trend === "up" ? TrendingUp : sig.trend === "down" ? TrendingDown : Minus
                return (
                  <div key={i} className="inline-flex items-center gap-2 font-sans"
                    style={{
